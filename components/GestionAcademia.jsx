...
const GestionAcademia = () => {
  ...
  useEffect(() => {
    const cargarEstudiantes = async () => {
      try {
        ...
        setModoOffline(true); // Asumimos modo offline siempre para evitar problemas
  ...
  const eliminarEstudianteHandler = async (id) => {
    try {
      ...
      setError('Error al eliminar el estudiante. Intente nuevamente.');
    } finally {
      setCargando(false);
    }
  };
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">Si tarda demasiado, se activará el modo offline</p>
        </div>
      </div>
    );
  }
                        ...
                        <tr key={`estudiante-${est.id}`}>
                          ...
                          <td>
                            <div className="flex gap-2">
                              <button 
                                className="button button-primary button-icon"onClick={() => {
                                  setEstudianteActual(est);
                                  setDatosPago({
                                    clasesPorPeriodo: est.clasesPorPeriodo || 8,
                                    frecuenciaClases: est.frecuenciaClases || 2
